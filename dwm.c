...
#define ISVISIBLEONTAG(C, T, S) ((C->tags & T) || S)
...
nexttagged(Client *c) {
	...
	for(;
		walked && (walked->isfloating || !ISVISIBLEONTAG(walked, c->tags, walked->issticky));
		walked = walked->next
	);
	return walked;
}
...
void
tagadjacent(const Arg *arg) {
	if(selmon->sel != NULL && __builtin_popcount(selmon->tagset[selmon->seltags] & TAGMASK) == 1) {
		if (arg->i > 0 && selmon->tagset[selmon->seltags] & (TAGMASK >> 1))
			selmon->sel->tags <<= 1;
